...
def _soft_clip(y: np.ndarray, threshold: float = 0.985) -> np.ndarray:
    """
    Avoid global gain pumping. Only soften samples that exceed the threshold.
    Samples below the threshold remain essentially unchanged.
    """
    ...
    if np.any(mask):
        excess = (abs_y[mask] - threshold) / max(1e-6, 1.0 - threshold)
...
def _render(
    ...
    time_scale = dry_duration / max(reference_duration, 1e-6)
    ...
    # One global stutter-layer gain, rather than separate loudness matching
    # for every fragment. This keeps the effect from breathing up and down.
    dry_rms = _rms(dry)
    target_wet_rms = dry_rms * 0.72

    for event in events:
        event_time = float(event["time"]) * time_scale
        ...
        # Grab the fragment ending at the mapped vocal event.
        ...
        # Remove DC only; do not normalize each snippet independently.
        ...
        # First repeat starts immediately after the source fragment.
        spacing = max(
            int(sr * interval_ms / 1000.0),
            int(len(snippet) * 0.82),
        )
        first_repeat = source_end + first_offset_samples
    ...
    wet_rms = _rms(wet)
    if wet_rms > 1e-9 and target_wet_rms > 1e-9:
        # One consistent gain for the entire layer.
        global_layer_gain = float(
            np.clip(target_wet_rms / wet_rms, 0.35, 1.75)
        )
        wet *= global_layer_gain
    ...
    with tempfile.TemporaryDirectory(prefix="vst_convert_") as temp_dir:
        ...
        if not events:
            ...
            fallback_times = np.linspace(
                ...
                num=min(
                    4,
                    max(1, int(reference_duration // 8) + 1),
                ),
            )
