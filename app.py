...
# Ensure the static website directory always exists on Render.
STATIC = BASE / "static"
STATIC.mkdir(parents=True, exist_ok=True)

# If index.html was uploaded at the repository root, copy it into /static.
root_index = BASE / "index.html"
static_index = STATIC / "index.html"

if not static_index.exists() and root_index.exists():
    import shutil
    shutil.copy2(root_index, static_index)

# Fallback page so the server can still start if index.html is missing.
...
<head>
  ...
  <title>Vocal Stutter Transfer</title>
</head>
<body style="font-family:Arial,sans-serif;background:#101114;color:white;text-align:center;padding:48px 20px">
  <h1>Vocal Stutter Transfer</h1>
